...
def fetch_answer_from_gpt(prompt):
    ...
    response = openai_client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": """You are an expert chef like Ranveer Brar, respond accordingly"""},
...
def comparison_page():
    ...
    if prompt:
        ...
        with st.spinner("Thinking (Ollama Llama3)"):
            ...
            llama3_result = ollama.chat(model="chef", messages=[{
                "role": "user",
                "content": prompt,
            }])
            end_time = time.time()
        ...
        with st.spinner("Thinking (Ollama Gemma:2b)"):
            start_time = time.time()
            gemma2b_result = ollama.chat(model="chef2", messages=[{
                "role": "user",
                "content": prompt,
            }])
            end_time = time.time()
